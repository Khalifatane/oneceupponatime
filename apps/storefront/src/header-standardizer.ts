...
import { supabase } from "@siggistore/supabase";
import {
  supabaseAuthService,
...
import * as storefrontStore from "./lib/store.js";

declare global {
  interface Window {
    __storefrontHeaderStandardizerLoaded?: boolean;
  }
}
...
(function () {
    ...
    if (!servicesPromise) {
      try {
        servicesPromise = Promise.resolve({
          ...
          orders: supabaseOrderService,
          cart: supabaseCartService,
          reviews: supabaseReviewService,
        });
      } catch (error) {
        console.error(
          "[header-standardizer] Failed to initialize services",
          error
        );
  ...
  function getSupabaseClient() {
    if (!supabasePromise) {
      try {
        supabasePromise = Promise.resolve(supabase);
      } catch (error) {
        console.error(
          "[header-standardizer] Failed to initialize Supabase client",
          error
        );
    ...
    if (!commerceStorePromise) {
      try {
        commerceStore = {
          ...
          normalizeCartItem: function (item, index) {
            return {
          ...
          getCart: function () {
            return readJsonStorage(cartItemsKey, []);
          },
          setCart: function (items) {
            writeJsonStorage(cartItemsKey, items);
            return items;
          },
          ...
          updateCartSummaryUI: updateCartSummaries,
          updateFavoritesSummaryUI: updateFavoritesSummary,
          bindAddToCartButtons: bindAddToCartButtons,
          bindFavoriteToggles: bindFavoriteToggles,
          extractProductContext: extractProductContext,
          hydrateProductDataAttributes: function (root) {
            // No-op: let consumers handle if needed
          },
          applyStorefrontDiscountPromos: function (root, options) {
            // Import and apply promos if needed
            return Promise.resolve();
          },
        };
        commerceStorePromise = Promise.resolve(commerceStore);
      } catch (error) {
        console.error(
          "[header-standardizer] Failed to initialize commerce store",
          error
        );
